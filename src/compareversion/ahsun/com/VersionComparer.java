...
public class VersionComparer {
	...
	private int getMaxRevisionLevels(String[] splittedVersion1, String[] splittedVersion2) {
		return splittedVersion1.length > splittedVersion2.length ? splittedVersion1.length : splittedVersion2.length;
	}

	private int[] getRevisionLevelsLengths(String[] splittedVersion1, String[] splittedVersion2,
			int maxRevisionLevels) {
	...
	private long convertVersionStringToInt(String[] splittedVersion, int[] revisionLevelLengths) {
		...
		for (var index = 0; index < revisionLevelLengths.length; index++) {
			if (index < splittedVersion.length) {
				...
				versionNumber.append(generateZerosString(revisionLevelLengths[index] - levelLength));
				versionNumber.append(revisionLevel);
				continue;
			}

			versionNumber.append(generateZerosString(revisionLevelLengths[index]));
		}

		return Long.parseLong(versionNumber.toString());
	}
